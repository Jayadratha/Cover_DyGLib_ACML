...
from models.TGAT_link_cover import TGAT
from models.MemoryModel_link_cover import MemoryModel, compute_src_dst_node_time_shifts # may need to change
...
from utils.loss import SelectiveLoss
...
if __name__ == "__main__":
    ...
    # get arguments
    args = get_link_prediction_args_cvr_based(is_evaluation=True)
    ...
    if args.model_name == 'EdgeBank':
        ...
        for run in range(args.num_runs):
            ...
            args.save_result_name_all = f'{args.negative_sample_strategy}_negative_sampling_{args.model_name}_all_{args.num_runs}_runs'
            ...
            load_model_folder = f"./saved_models/{args.model_name}/{args.dataset_name}/{args.load_model_name}/lr_{args.learning_rate}/link_cov_{args.coverage}/"
            ...
            save_result_folder = f"./saved_results/{args.model_name}/{args.dataset_name}/lr_{args.learning_rate}/link_cov_{args.coverage}/"
        ...
        # save model result for all runs
        if args.model_name not in ['JODIE', 'DyRep', 'TGN']:
            result_json_all = {
                ...
                "average new node test metrics": {metric_name: f'{np.mean([new_node_test_metric_single_run[metric_name] for new_node_test_metric_single_run in new_node_test_metric_all_runs]):.4f} '
                                                f'± {np.std([new_node_test_metric_single_run[metric_name] for new_node_test_metric_single_run in new_node_test_metric_all_runs], ddof=1):.4f}' for metric_name in new_node_test_metric_all_runs[0]},                
                ...
                "average new node validate metrics": {metric_name: f'{np.mean([new_node_val_metric_single_run[metric_name] for new_node_val_metric_single_run in new_node_val_metric_all_runs]):.4f} '
                                                    f'± {np.std([new_node_val_metric_single_run[metric_name] for new_node_val_metric_single_run in new_node_val_metric_all_runs], ddof=1):.4f}' for metric_name in new_node_val_metric_all_runs[0]},
                "test metrics": {metric_name: [test_metric_single_run[metric_name] for test_metric_single_run in test_metric_all_runs] for metric_name in test_metric_all_runs[0]},
                "new node test metrics": {metric_name: [new_node_test_metric_single_run[metric_name] for new_node_test_metric_single_run in new_node_test_metric_all_runs] for metric_name in new_node_test_metric_all_runs[0]},
                "validate metrics": {metric_name: [val_metric_single_run[metric_name] for val_metric_single_run in val_metric_all_runs] for metric_name in val_metric_all_runs[0]},
                "new node validate metrics": {metric_name: [new_node_val_metric_single_run[metric_name] for new_node_val_metric_single_run in new_node_val_metric_all_runs] for metric_name in new_node_val_metric_all_runs[0]}
        ...
        save_result_folder = f"./saved_results/{args.model_name}/{args.dataset_name}/lr_{args.learning_rate}/link_cov_{args.coverage}/"
        os.makedirs(save_result_folder, exist_ok=True)
        save_result_path = os.path.join(save_result_folder, f"{args.save_result_name_all}.json")
        with open(save_result_path, 'w') as file:
            file.write(result_json_all)
        logger.info(f'save negative sampling results at {save_result_path} for all {args.num_runs} runs.')
