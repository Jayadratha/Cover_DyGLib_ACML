...
def evaluate_model_node_classification(model_name: str, model: nn.Module, neighbor_sampler: NeighborSampler, evaluate_idx_data_loader: DataLoader,
    ...
    with torch.no_grad():
        ...
        for batch_idx, evaluate_data_indices in enumerate(evaluate_idx_data_loader_tqdm):
            ...
            tsne_embedding.append(batch_src_node_embeddings) ## it is coming from the TG encoder, so never changes?
        ...
        tsne_embedding = torch.cat(tsne_embedding, dim=0)
        labels_cpu = evaluate_y_trues.clone().cpu()


        tsne_embedding_cpu = tsne_embedding.cpu()

        # # Train TSNE
        # tsne = TSNE(n_components=2, learning_rate='auto',
        #          init='pca').fit_transform(tsne_embedding_cpu.detach())
        # # randomly take 2000 samples when label is 1
        # # take all samples when label is 0
        # tsne_label_1 = tsne[labels_cpu == 1]
        # tsne_label_0 = tsne[labels_cpu == 0]
        # print("tsne_label_1.shape: ", tsne_label_1.shape, "tsne_label_0.shape: ", tsne_label_0.shape)
        # tsne_label_0_2000 = tsne_label_0[random.sample(range(0, tsne_label_0.shape[0]), 2000)]

        # # tsne = np.vstack((tsne_label_0[random.sample(range(0, tsne_label_0.shape[0]), 2000)], tsne_label_1))
        # print("tsne.shape: ", tsne.shape)
        # labels_taken_cpu = np.concatenate([np.zeros(2000), np.ones(tsne_label_1.shape[0])])

        # print("labels_taken_cpu.shape: ", labels_taken_cpu.shape)


        # # # Plot TSNE
        # plt.figure(figsize=(10, 10))
        # plt.axis('off')
        # # plt.scatter(tsne[:, 0], tsne[:, 1], s=50, c=labels_taken_cpu)
        # # plot red color for label 1, blue color for label 0
        # plt.scatter(tsne_label_1[:, 0], tsne_label_1[:, 1], c='r', s=90)
        # plt.scatter(tsne_label_0_2000[:, 0], tsne_label_0_2000[:, 1], c='b', s=50)
        # # plt.scatter(tsne_label_neg1[:, 0], tsne_label_neg1[:, 1], c='k', s=1)
        # # plt.colorbar()
        # # seed = random.randint(0, 1000)

        # plt.title(f'TSNE of node embeddings for {model_name}')

        # save_folder = f'./visuals'
        # os.makedirs(save_folder, exist_ok=True)
        # # save the plot
        # plt.savefig(f'./visuals/tsne_cnt_1_{np.count_nonzero(labels_taken_cpu == 1)}_cnt_0_{np.count_nonzero(labels_taken_cpu == 0)}_rnd_seed_{random.randint(0, 1000)}.png')
