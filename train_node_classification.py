...
from models.modules_orig import MergeLayer, MLPClassifier
from utils.utils import set_random_seed, convert_to_gpu, get_parameter_sizes, create_optimizer
from utils.utils import get_neighbor_sampler
from evaluate_models_utils_orig import evaluate_model_node_classification
...
from utils.load_configs_orig import get_node_classification_args
...
if __name__ == "__main__":
    ...
    for run in range(args.num_runs):
        ...
        save_result_path = os.path.join(save_result_folder, f"{args.save_model_name}_lr{args.learning_rate}_orig_100_cov.json")
    ...
    if args.model_name not in ['JODIE', 'DyRep', 'TGN']:
        val_metric_avg_dict, test_metric_avg_dict = {}, {}
        for metric_name in val_metric_all_runs[0].keys():
            val_metric_avg = np.mean([val_metric_single_run[metric_name] for val_metric_single_run in val_metric_all_runs])
            val_metric_avg_dict[metric_name] = val_metric_avg
    for metric_name in test_metric_all_runs[0].keys():
        test_metric_avg = np.mean([test_metric_single_run[metric_name] for test_metric_single_run in test_metric_all_runs])
        test_metric_avg_dict[metric_name] = test_metric_avg

    # if args.model_name not in ['JODIE', 'DyRep', 'TGN']:
    #     result_avg_json = {
    #         "validate metrics": {metric_name: f'{val_metric_avg_dict[metric_name]:.4f}' for metric_name in val_metric_avg_dict},
    #         "test metrics": {metric_name: f'{test_metric_avg_dict[metric_name]:.4f}' for metric_name in test_metric_avg_dict}
    #     }
    # else:
    #     result_avg_json = {
    #         "test metrics": {metric_name: f'{test_metric_avg_dict[metric_name]:.4f}' for metric_name in test_metric_avg_dict}
    #     }
    # result_avg_json = json.dumps(result_avg_json, indent=4)


    # save_result_avg_path = os.path.join(save_result_folder, f"{args.save_model_name}_lr{args.learning_rate}_orig_100_cov_avg_{args.num_runs}_runs.json")

    # with open(save_result_avg_path, 'w') as file:
    #     file.write(result_avg_json)
