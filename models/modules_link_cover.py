...
class MergeLayer(nn.Module):
    ...
    def __init__(self, input_dim1: int, input_dim2: int, hidden_dim: int, output_dim: int):
        ...
        self.sel_head = nn.Sequential(nn.Linear(output_dim, 128), nn.BatchNorm1d(128), nn.ReLU(), nn.Linear(128, 1), nn.Sigmoid()) # single sigmoid output
    ...
    def forward(self, input_1: torch.Tensor, input_2: torch.Tensor):
        ...
        h = self.fc2(self.act1(self.fc1(x)))
        sel_head = self.sel_head(h) # sel_head: [batch_size * 2, 1] 
        aux = self.act2(h) # aux: [batch_size * 2, 1]        
        return h, sel_head, aux
...
class MLPClassifier(nn.Module):
    def __init__(self, input_dim: int, dropout: float = 0.1):
        ...
        self.sel_head = nn.Sequential(nn.Linear(1, 128), nn.BatchNorm1d(128), nn.ReLU(), nn.Linear(128, 1), nn.Sigmoid()) # single sigmoid output
    ...
    def forward(self, x: torch.Tensor):
        ...
        """
        # Tensor, shape (*, 80)
        x = self.dropout(self.act_2(self.fc1(x)))
        # Tensor, shape (*, 10)
        x = self.dropout(self.act_2(self.fc2(x)))
        # Tensor, shape (*, 1)
        emb = self.fc3(x)
        decoder = emb.squeeze(dim=1) # decoder: (batch_size(= 100), )
        # sigmoid has been applied later where decoder embeding is used
        sel_head = self.sel_head(emb).squeeze(dim=1) # sel_head: (batch_size(= 100), )
        aux = (self.act_1(emb)).squeeze(dim=1) # aux: (batch_size(= 100), )        

        return decoder, sel_head, aux
        # return self.fc3(x)
