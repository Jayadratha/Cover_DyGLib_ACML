...
if __name__ == "__main__":
    ...
    for run in range(args.num_runs):
        ...
        args.save_result_name_all = f'evaluate_node_classification_{args.model_name}_all_{args.num_runs}_runs'
        ...
        os.makedirs(f"./logs/{args.model_name}/{args.dataset_name}/{args.save_result_name}/lr_{args.learning_rate}/cov_{args.coverage}/", exist_ok=True)
    ...
    # store the average metrics at the log of the all runs
    if args.model_name not in ['JODIE', 'DyRep', 'TGN']:
        result_json_all = {
            "validate metrics for all {args.num_runs} runs": {metric_name: f'{np.mean([val_metric_single_run[metric_name] for val_metric_single_run in val_metric_all_runs]):.4f} ± {np.std([val_metric_single_run[metric_name] for val_metric_single_run in val_metric_all_runs], ddof=1):.4f}' for metric_name in val_metric_all_runs[0]},
            "test metrics for all {args.num_runs} runs": {metric_name: f'{np.mean([test_metric_single_run[metric_name] for test_metric_single_run in test_metric_all_runs]):.4f} ± {np.std([test_metric_single_run[metric_name] for test_metric_single_run in test_metric_all_runs], ddof=1):.4f}' for metric_name in test_metric_all_runs[0]},
            # also include results for individual runs
            "validate metrics for all individual {args.num_runs} runs": {metric_name: [val_metric_single_run[metric_name] for val_metric_single_run in val_metric_all_runs] for metric_name in val_metric_all_runs[0]},
            "test metrics for all individual {args.num_runs} runs": {metric_name: [test_metric_single_run[metric_name] for test_metric_single_run in test_metric_all_runs] for metric_name in test_metric_all_runs[0]}
        }
    else:
        result_json_all = {
            "test metrics for all {args.num_runs} runs": {metric_name: f'{np.mean([test_metric_single_run[metric_name] for test_metric_single_run in test_metric_all_runs]):.4f} ± {np.std([test_metric_single_run[metric_name] for test_metric_single_run in test_metric_all_runs], ddof=1):.4f}' for metric_name in test_metric_all_runs[0]},
    ...
    save_result_folder_all = f"./saved_results/{args.model_name}/{args.dataset_name}/lr_{args.learning_rate}/cov_{args.coverage}"
    os.makedirs(save_result_folder_all, exist_ok=True)
    save_result_path_all = os.path.join(save_result_folder_all, f"{args.save_result_name_all}.json")
    with open(save_result_path_all, 'w') as file:
        file.write(result_json_all)
    logger.info(f'save node classification results at {save_result_path_all} for all {args.num_runs} runs')
